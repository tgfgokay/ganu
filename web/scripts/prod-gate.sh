...
#   2) Gerçek owner/admin JWT ile admin RPC çağrısı bu koşuda yapılmalı; dönen
#      nonce bağlı HMAC kanıtı CI tarafında doğrulanmalı.
#   3) DB readiness kontrolü owner/admin ve aynı RPC'nin audit kaydını doğrulamalı.
...
command -v xxd >/dev/null || { echo "PROD GATE FAIL: xxd bulunamadı." >&2; exit 2; }
...
# 2) DB dışı, nonce-bağlı kriptografik kanıt. SQL Editor'de tabloya kayıt
...
expected="$(printf '%s' "$payload" | openssl dgst -sha256 -hmac "$PROD_GATE_HMAC_SECRET" -binary | xxd -p -c 256)"
if [ "$signature" != "$expected" ]; then
  echo "PROD GATE FAIL: admin-gate HMAC imzası doğrulanamadı." >&2
  exit 3
fi

# 3) DB gate (owner/admin + audit kaydı)
