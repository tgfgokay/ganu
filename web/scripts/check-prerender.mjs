...
const {render,indexableRoutes}=await import(`${pathToFileURL(path.resolve('dist-ssr/entry-server.js')).href}?check=${Date.now()}`)
const routes=indexableRoutes(),expected=new Set(routes.map((route)=>config.absolute(route.path)))
...
for(const route of routes){
  ...
  const tr=one(head,/<link rel="alternate" hreflang="tr" href="([^"]+)">/g,`${route.path} tr`)
  const en=one(head,/<link rel="alternate" hreflang="en" href="([^"]+)">/g,`${route.path} en`)
  const xd=one(head,/<link rel="alternate" hreflang="x-default" href="([^"]+)">/g,`${route.path} x-default`)
  const other=routes.find((candidate)=>candidate.path===route.counterpart)
  if(!other||tr!==(route.locale==='tr'?canonical:config.absolute(other.path))||en!==(route.locale==='en'?canonical:config.absolute(other.path))||xd!==tr||other.counterpart!==route.path)throw new Error(`${route.path}: hreflang reciprocal`)
...
if(urlBlocks.length!==routes.length)throw new Error('sitemap url block sayısı')
for(const route of routes){
  ...
  const other=config.absolute(route.counterpart),tr=route.locale==='tr'?canonical:other,en=route.locale==='en'?canonical:other
  const alternates=[...block.matchAll(/<xhtml:link rel="alternate" hreflang="([^"]+)" href="([^"]+)"\/>/g)].map((m)=>[m[1],m[2]])
  const wanted=[['tr',tr],['en',en],['x-default',tr]]
