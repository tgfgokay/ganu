...
if(assets.some((name)=>/^(?:PanelApp|SatinAl|MusteriPortal|OrtakPortal|panel|supabase-vendor)-/.test(name)))throw new Error('private/Supabase chunk üretildi')
const built=assets.map((name)=>fs.readFileSync(path.join(dist,'assets',name),'utf8')).join('\n')
for(const marker of ['supabase.co','purchase-flow','pos-payment','get-file','createClient'])if(built.includes(marker))throw new Error(`marketing runtime integration marker: ${marker}`)
