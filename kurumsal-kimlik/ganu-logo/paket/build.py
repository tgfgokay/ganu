#!/usr/bin/env python3
# GANU — FİNAL v3: Navy + Teal, Fraunces serif, tek teal nokta aksanı.
# Palet: Navy #0A2540 (metin/yapı) + Teal #00D4B2 (tek aksan) + Beyaz/Açık gri (zemin).
# Klişe ikon YOK; wordmark + soyut teal nokta. Mono (teal'siz) versiyon tek-renk baskı için.
# NOT: Önceki iki-ton (Kemik+Lacivert) paket arsiv/paket-iki-ton-altinsiz/ altında.
...
# ============== MERKEZ KİLİT (ANA LOGO) — wordmark + teal nokta ==============
def dikey(bg, fg):
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 520 240">
  <rect width="520" height="240" fill="{bg}"/>
  <text x="252" y="128" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="96" letter-spacing="2" fill="{fg}">GANU</text>
  <circle cx="420" cy="122" r="9" fill="{TEAL}"/>
  <text x="256" y="172" text-anchor="middle" font-family="{SANS}" font-weight="500" font-size="12" letter-spacing="12" fill="{fg}">SANAL OFİS<tspan fill="{TEAL}" dx="4">·</tspan><tspan dx="4">İSTANBUL</tspan></text>
</svg>'''
w("ganu-dikey",      dikey(LGRAY, NAVY))
w("ganu-dikey-koyu", dikey(NAVY,  WHITE))

# ============== YATAY (NAVBAR) — wordmark + teal nokta | descriptor ==============
def yatay(bg, fg, rule):
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 460 110">
  <rect width="460" height="110" fill="{bg}"/>
  <text x="34" y="72" font-family="{SERIF}" font-weight="600" font-size="58" letter-spacing="1" fill="{fg}">GANU</text>
  <circle cx="226" cy="66" r="6" fill="{TEAL}"/>
  <line x1="256" y1="32" x2="256" y2="78" stroke="{rule}" stroke-width="1.3"/>
  <text x="274" y="50" font-family="{SANS}" font-weight="600" font-size="12" letter-spacing="5" fill="{fg}">SANAL OFİS</text>
  <text x="274" y="71" font-family="{SANS}" font-weight="500" font-size="11" letter-spacing="6.5" fill="{fg}">İSTANBUL</text>
</svg>'''
w("ganu-yatay",      yatay(LGRAY, NAVY, "#0A254066"))
w("ganu-yatay-koyu", yatay(NAVY,  WHITE, "#FFFFFF55"))

# ============== AVATAR / AMBLEM — "G" + teal nokta ==============
def avatar(bg, g, border=False):
  ...
  <text x="288" y="416" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="368" fill="{g}">G</text>
  <circle cx="430" cy="388" r="26" fill="{TEAL}"/>
</svg>'''
w("ganu-avatar",      avatar(NAVY, WHITE))
w("ganu-avatar-acik", avatar(LGRAY, NAVY, border=True))

# ============== FAVICON ==============
favicon = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 64 64">
  <rect width="64" height="64" rx="14" fill="{NAVY}"/>
  <text x="30" y="47" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="46" fill="{WHITE}">G</text>
  <circle cx="47" cy="42" r="5" fill="{TEAL}"/>
</svg>'''
w("ganu-favicon", favicon)

# ============== KAŞE / MÜHÜR (resmi belge) — tek renk navy ==============
kase = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 600 600">
  <rect width="600" height="600" fill="{LGRAY}"/>
  ...
  <text x="280" y="322" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="112" letter-spacing="3" fill="{NAVY}">GANU</text>
  <circle cx="452" cy="312" r="9" fill="{TEAL}"/>
...
# ============== MONO (tek renk navy, teal YOK — tek-renk baskı/faks) ==============
def mono(bg, fg):
    return f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 520 240">
  <rect width="520" height="240" fill="{bg}"/>
  <text x="252" y="128" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="96" letter-spacing="2" fill="{fg}">GANU</text>
  <circle cx="420" cy="122" r="9" fill="{fg}"/>
  <text x="256" y="172" text-anchor="middle" font-family="{SANS}" font-weight="500" font-size="12" letter-spacing="12" fill="{fg}">SANAL OFİS<tspan dx="4">·</tspan><tspan dx="4">İSTANBUL</tspan></text>
</svg>'''
...
kart_on = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}">
  ...
  <text x="{W/2-12}" y="170" text-anchor="middle" font-family="{SERIF}" font-weight="600" font-size="78" letter-spacing="2" fill="{WHITE}">GANU</text>
  <circle cx="{W/2+108}" cy="164" r="8" fill="{TEAL}"/>
  <text x="{W/2}" y="205" text-anchor="middle" font-family="{SANS}" font-weight="500" font-size="11" letter-spacing="10" fill="{WHITE}">SANAL OFİS<tspan fill="{TEAL}" dx="3">·</tspan><tspan dx="3">İSTANBUL</tspan></text>
</svg>'''
w("kartvizit-on", kart_on)

kart_arka = f'''<svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 {W} {H}">
  <rect width="{W}" height="{H}" fill="{LGRAY}"/>
  <text x="44" y="80" font-family="{SERIF}" font-weight="600" font-size="40" letter-spacing="1" fill="{NAVY}">GANU</text>
  <circle cx="172" cy="74" r="5" fill="{TEAL}"/>
  <line x1="46" y1="100" x2="92" y2="100" stroke="{TEAL}" stroke-width="3"/>
...
r("ganu-mono","ganu-mono@2x.png",["-z","2"]); r("ganu-mono-koyu","ganu-mono-koyu@2x.png",["-z","2"])
